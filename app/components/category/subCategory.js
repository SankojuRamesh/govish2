  ...
  return (
    <>
      {categories.map((category, index) => (
        <Grid size={{ md: 3, sm: 6, xs: 12 }} key={index} className="mt-8">
          <Box className="position-relative">
            ...
            <Link
              href="#"
              className="span-none    h-auto w-auto p-0 min-wa ms-2 mb-2 position-absolute bottom-0 right-0 fs-4 text-white"
            >
              <FaPlayCircle />
            </Link>
          </Box>
          <CardContent className="p-0 mt-3 ">
            <Link
              href="#"
              className="text-decoration-none fs-6 text-light-emphasis fw-medium"
            >
              {category.title}
            </Link>
            <p className="text-black text-opacity-25 fw-medium mt-1 mb-0">
              {category.subtitle}
            </p>
          </CardContent>
        </Grid>
